...
- Reduce memory usage with mutations in a pending txn by only keep deltas in memory for a pending txn. ([#3349][])
- Reduce memory usage by limiting the number of pending proposals in apply channel. ([#3340][])
- Reduce memory usage by retrieving entries in batches when calculating a snapshot. ([#3409][])
...
- Remove incorrect `dgraph_conf` from /debug/vars. Dgraph config options are now shown in the logs. ([#3337][])
