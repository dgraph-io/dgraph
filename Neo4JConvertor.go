...
func main() {
	...
	//open the file
	ifile, err := os.Open(*inputPath) //removed err here
	...
	oFile, err := os.OpenFile(outputName, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0600) //removed err
	check(err)
	defer oFile.Close()
	//process the file
	check(processNeo4jCSV(ifile, oFile))
	fmt.Printf("Finished writing %q\n", outputName)
...
func processNeo4jCSV(r io.Reader, w io.Writer) error {
	...
	for scanner.Scan() {
		...
		if err != nil {
			ErrQuote := fmt.Sprint(err)
			if strings.Contains(ErrQuote, "extraneous or missing \" in quoted-field") {
				//ignore the error for missing "
				continue
			} else {
				glog.Error(err)
				check(err)
		...
		for position := 0; position < len(line); position++ {
			...
			if len(line[0]) > 0 {
				...
				if position < positionOfStart && position > 0 {
					//Replace the new line in a query with '\n'

					resultNewString := fmt.Sprintf(line[position])
					resultNewString = strings.Replace(resultNewString, "\n", `\n`, -1)

					//write non-facet data

					rdfLines.WriteString(fmt.Sprintf("%s <%s> \"%s\" .\n",
						bn, header[position], resultNewString))
				}
				continue
			}
			// Handle relationship data.
			if position >= positionOfStart {
				//fmt.Println(line[position]) handling only facets
