...
func IntersectWithLinPacked(uidPack1, uidPack2 *pb.UidPack) *pb.UidPack {
	...
	for dec1Idx := 0; dec1Idx < len(dec1.Pack.Blocks); dec1Idx++ {
		...
		if uidPack1Msb == uidPack2Msb {
			dec1Block := dec1.RoaringBitmapForBlock(dec1Idx)
			dec2Block := dec2.RoaringBitmapForBlock(dec2Idx)
			rb := gocroaring.And(dec1Block, dec2Block)
			//C.free(dec1Block)
			//C.free(dec2Block)
			//intersection.AddBlockFromBitmap(uidPack1Msb, rb, uint32(rb.GetCardinality()))
			intersection.AddBlockFromBitmap(uidPack1Msb, rb, uint32(rb.Cardinality()))
		}
...
func DifferencePacked(uidPack1, uidPack2 *pb.UidPack) *pb.UidPack {
	...
	for dec1Idx := 0; dec1Idx < len(dec1.Pack.Blocks); dec1Idx++ {
		...
		if uidPack1Msb == uidPack2Msb {
			rb := gocroaring.AndNot(dec1.RoaringBitmapForBlock(dec1Idx), dec2.RoaringBitmapForBlock(dec2Idx))
			//difference.AddBlockFromBitmap(uidPack1Msb, rb, uint32(rb.GetCardinality()))
			difference.AddBlockFromBitmap(uidPack1Msb, rb, uint32(rb.Cardinality()))
		} else {
