...
func fillNums(commonNums, blockNums, arrNums []uint64, N1, N2 int) {
...
func TestIntersectCompressedWithLinJump(t *testing.T) {
	...
	for _, N1 := range NUM1 {
		for _, N2 := range NUM2 {
			...
			arrNums := make([]uint64, N1+N2)

			fillNums(commonNums[:], blockNums[:], arrNums[:], N1, N2)
			...
			sort.Slice(arrNums, func(i, j int) bool { return arrNums[i] < arrNums[j] })

			o := make([]uint64, 0)
			IntersectCompressedWithLinJump(&dec, arrNums, &o)
