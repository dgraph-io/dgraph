...
// without -tags=requirefips and with DGRAPH_FIPS_BINARY unset, FIPSEnabled
// returns false. Under -tags=requirefips, the tag-guarded init in
// buildvars/fips_requirefips.go flips fipsEnabledCompiled to true and the
// untagged build of this test isn't compiled — so it's safe to assert
...
// FIPSEnabled() returns true only when DGRAPH_FIPS_BINARY is set to
// exactly "1". Covers the common edge cases: unset, empty, "0", "1",
// and other non-"1" values.
...
// saveAndUnsetEnv is a small test helper that captures the current value
// of name (if any), unsets it, and registers a cleanup to restore the
// original state. Avoids leaking test-set env values across cases.
