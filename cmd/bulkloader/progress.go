...
func (p *progress) reportOnce() {
	...
	case mapPhase:
		...
		fmt.Printf("MAP %s rdf_count:%s rdf_speed:%s/sec edge_count:%s edge_speed:%s/sec DEBUG(MR:%d MY:%d LBC:%d)\n",
			...
			atomic.LoadInt64(&p.mappersYielding),
			p.lenBatchCh(),
		)
	case reducePhase:
		now := time.Now()
