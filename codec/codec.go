...
func (e *Encoder) AddBlockFromBitmap(base uint64, rb *gocroaring.Bitmap, size uint32) {
	if e.pack == nil {
		e.pack = &pb.UidPack{}
	}
	encData := make([]byte, rb.SerializedSizeInBytes())
	x.Check(rb.Write(encData))
...
func (e *Encoder) packBlock() {
	...
	roaringBitmap := gocroaring.New()
	//roaringBitmap.AddMany(e.uids)
	roaringBitmap.Add(e.uids...)
...
type Decoder struct {
	...
	blockIdx      int
	uids          []uint64
	RoaringBitmap *gocroaring.Bitmap
}
...
func (d *Decoder) RoaringBitmapForBlock(blockIdx int) *gocroaring.Bitmap {
	block := d.Pack.Blocks[blockIdx]
	//rb := gocroaring.New()
	//x.Check2(rb.FromBuffer(block.Deltas))
	rb, err := gocroaring.Read(block.Deltas)
	x.Check(err)
	return rb
}
...
func (d *Decoder) UnpackBlock() []uint64 {
	...
	/*
	if d.RoaringBitmap != nil {
		C.free(d.RoaringBitmap)
	}
	*/
	rb, err := gocroaring.Read(block.Deltas)
	x.Check(err)
	...
	d.uids = make([]uint64, d.RoaringBitmap.Cardinality())
