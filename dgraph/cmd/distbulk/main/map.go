...
func emitMapEntry(pred string, uid interface{}, key []byte, posting *pb.Posting) error {
    pbytes, err := proto.Marshal(posting)
    if err != nil {
        return err
    }
    gio.Emit(pred, uid, key, pbytes)
    return nil
}
...
func processNQuad(nq gql.NQuad) error {
    ...
    err = emitMapEntry(nq.Predicate, sid, key, fwd)
    if err != nil {
        return err
    }
	...
	if rev != nil {
        ...
        err = emitMapEntry(nq.Predicate, oid, key, rev)
        if err != nil {
            return err
        }
	}
	...
	if Opt.ExpandEdges {
        ...
        err = emitMapEntry("_predicate_", sid, key, pp)
        if err != nil {
            return err
        }
	}
...
func lookupUid(xid string) (uid uint64, err error) {
    err = Xidmap.View(func(txn *badger.Txn) error {
...
func addIndexMapEntries(nq gql.NQuad, de *pb.DirectedEdge) error {
	...
	for _, tokerName := range sch.GetTokenizer() {
		...
		for _, t := range toks {
            ...
            err = emitMapEntry(nq.Predicate, t, key, p)
            if err != nil {
                return err
            }
		}
...
func createPredicatePosting(predicate string) *pb.Posting {
	...
	fp := farm.Fingerprint64(predbytes)
	return &pb.Posting{
		Uid:         fp,
		Value:       predbytes,
