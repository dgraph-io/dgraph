...
func GenerateCompleteSchema(schema *ast.Schema) *gqlerror.Error {
...
func addQueryType(defn *ast.Definition) []*ast.FieldDefinition {
	flds := make([]*ast.FieldDefinition, 0)
...
func addMutationType(defn *ast.Definition) []*ast.FieldDefinition {
	flds := make([]*ast.FieldDefinition, 0)
...
func generateQMString(flag bool, qry *ast.Definition) string {
	...
	for _, fld := range qry.Fields {
		...
		for _, arg := range fld.Arguments {
			argStrs = append(argStrs, arg.Name+": "+arg.Type.String())
		}
