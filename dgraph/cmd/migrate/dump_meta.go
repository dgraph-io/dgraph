...
// Its predicate name is constructed by concatenating the table name, and each column's name in alphabetical order
// The object _:person_2 is the blank node label from the person table,
// and it's generated by a lookup using the key, called "ref label", person_company_Google_employee_id_100.
// The mapping from the ref label person_company_Google_employee_id_100 to the foreign blank node _:person_2
...
func getRefLabelFromConstraint(rowMetaInfo *RowMetaInfo,
	...
	if constraint.foreignColumnIndices == nil {
		...
		constraint.foreignColumnIndices = getColumnIndices(tableInfo, func(info *TableInfo, column string) bool {
