...
func TestIdBump(t *testing.T) {
	...
	// Bump the lease to 1,000,000.
	res, err = zc.AssignIds(ctx, &pb.Num{Val: 1000000, Type: pb.Num_UID, Bump: true})
	require.NoError(t, err)

	// Next assignemnt's startId should be greater than 1,000,000.
	res, err = zc.AssignIds(ctx, &pb.Num{Val: 10, Type: pb.Num_UID})
	require.NoError(t, err)
	require.Greater(t, res.GetStartId(), uint64(1000000))
	require.Equal(t, uint64(10), res.GetEndId()-res.GetStartId()+1)
