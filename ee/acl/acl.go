...
func chMod(conf *viper.Viper) error {
	...
	{
		var(func: eq(dgraph.xid, "%s")) @filter(type(dgrpah.type.Group)) {
			gUID as uid
...
func queryUser(ctx context.Context, txn *dgo.Txn, userid string) (user *User, err error) {
    ...
    query search($userid: string){
      user(func: eq(dgraph.xid, $userid)) @filter(type(dgrpah.type.User)) {
	    uid
        dgraph.xid
        dgraph.user.group {
          uid
...
func queryGroup(ctx context.Context, txn *dgo.Txn, groupid string,
	...
	query := fmt.Sprintf(`
		query search($groupid: string){
			group(func: eq(dgraph.xid, $groupid)) @filter(type(dgrpah.type.Group)) {
				uid
				%s
			}
		}`, strings.Join(fields, ", "))
