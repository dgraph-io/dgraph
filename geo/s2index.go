...
func loopFromPolygon(p *geom.Polygon) (*s2.Loop, error) {
	...
	for i := 0; i < n-1; i++ {
		c := r.Coord(i)
		pts = append(pts, pointFromCoord(c))
	}
...
func (l loopRegion) edgesCross(c s2.Cell) bool {
	for i := 0; i < 4; i++ {
		crosser := s2.NewChainEdgeCrosser(c.Vertex(i), c.Vertex(i+1), l.Vertex(0))
...
func coverLoop(l *s2.Loop, minLevel int, maxLevel int, maxCells int) s2.CellUnion {
	...
	// TODO: Determine if FastCovering can be used
