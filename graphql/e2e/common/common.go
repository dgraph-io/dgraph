...
func addModelSpec(url, modelId, modelEndpoint, modelType string) error {
	add := &GraphQLParams{
		Query: `mutation addModelSpec($id: String!, $endpoint: String!, $type: String!) {
			addModelSpec(input: { id: $id, type: $type, endpoint: $endpoint }) {
				modelSpec {
					id
					type
					endpoint
		...
		Variables: map[string]interface{}{"id": modelId, "endpoint": modelEndpoint, "type": modelType},
	}
	req, err := add.CreateGQLPost(url)
	if err != nil {
		return errors.Wrap(err, "error creating GraphQL query")
	...
	if addResult.Data.AddModelSpec.ModelSpec[0].ID != modelId ||
		addResult.Data.AddModelSpec.ModelSpec[0].Type != modelType ||
		addResult.Data.AddModelSpec.ModelSpec[0].Endpoint != modelEndpoint {
		return errors.New("GraphQL addModelSpec mutation failed")
	}

	return nil
}

func addEmbeddingSpec(url, id, entityType, predicate, query, prompt, modelId, modelType, modelEndpoint string) error {
	add := &GraphQLParams{
		Query: `mutation addEmbeddingSpec($id: String!, $entityType: String!, $predicate: String!, $query: String!, 
			$prompt: String!, $modelId: String!, $modelType: String!, $modelEndpoint: String!) {
			addEmbeddingSpec(input: { id: $id, entityType: $entityType, predicate: $predicate, query: $query, 
				prompt: $prompt, modelSpec: { id: $modelId, type: $modelType, endpoint: $modelEndpoint } }) {
				embeddingSpec {
					id
					entityType
					predicate
	...
	if addResult.Data.AddEmbeddingSpec.EmbeddingSpec[0].ID != id ||
		addResult.Data.AddEmbeddingSpec.EmbeddingSpec[0].EntityType != entityType ||
		addResult.Data.AddEmbeddingSpec.EmbeddingSpec[0].Predicate != predicate ||
		addResult.Data.AddEmbeddingSpec.EmbeddingSpec[0].Query != query ||
		addResult.Data.AddEmbeddingSpec.EmbeddingSpec[0].Prompt != prompt ||
		addResult.Data.AddEmbeddingSpec.EmbeddingSpec[0].ModelSpec.ID != modelId ||
		addResult.Data.AddEmbeddingSpec.EmbeddingSpec[0].ModelSpec.Type != modelType ||
		addResult.Data.AddEmbeddingSpec.EmbeddingSpec[0].ModelSpec.Endpoint != modelEndpoint {
		return errors.New("GraphQL addEmbeddingSpec mutation failed")
	}
