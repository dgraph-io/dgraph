...
func (mr *dgraphResolver) rewriteAndExecute(
	...
	case *UpdateRewriter:
		...
		xidMd := newXidMetadata()
		var queries []*gql.GraphQuery
		queries, err = NewRewriteUpdate(ctx, mutation, varGen, xidMd)
		if err != nil {
			return emptyResult(schema.GQLWrapf(err, "couldn't rewrite mutation %s", mutation.Name())),
				resolverFailed
		}
		...
		// Create mutations and new nodes depending on result of queries.
		upserts, setFrags, delFrags, err = NewUpdateMutations(ctx, mutation, varGen, xidMd, qNameToUID)
	...
	case *UpdateRewriter:
		dgQuery, err = NewFromMutationResultUpdate(ctx, mutation, mutResp.GetUids(), result, setFrags, delFrags)
	default:
