...
func (hr *httpResolver) rewriteAndExecute(
	...
	if err != nil {
		return nil, x.GqlErrorf("Evaluation of custom field failed because external request "+
			"failed with error: %s for field: %s.", err,
			query.Name()).WithLocations(query.Location())
	}
	return b, nil
}
