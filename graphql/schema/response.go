...
func (r *Response) WithError(err error) {
	...
	if !x.AlphaConfig.GraphQLDebug && strings.Contains(err.Error(), "authorization failed") {
		return
	}

	if !x.AlphaConfig.GraphQLDebug && strings.Contains(err.Error(), "GraphQL debug:") {
		return
	}
...
func (r *Response) Output() interface{} {
	...
	if x.AlphaConfig.GraphQL.GetBool("extensions") {
