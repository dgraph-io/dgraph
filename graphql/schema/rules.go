...
func apolloRequiresValidation(sch *ast.Schema,
	...
	if extendsDirective == nil {
		return []*gqlerror.Error{gqlerror.ErrorPosf(
			dir.Position,
			"Type %s: Field %s: @requires directive can only be defined on fields in type extensions. i.e., "+
				"the type must have `@extends` or use `extend` keyword.",
			typ.Name,
			field.Name,
		)}
	}
...
func apolloExternalValidation(sch *ast.Schema,
	...
	if extendsDirective == nil {
		return []*gqlerror.Error{gqlerror.ErrorPosf(
			dir.Position,
			"Type %s: Field %s: @external directive can only be defined on fields in type extensions. i.e., "+
				"the type must have `@extends` or use `extend` keyword.",
			typ.Name,
			field.Name,
		)}
