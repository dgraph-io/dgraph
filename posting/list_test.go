...
func listToArray(t *testing.T, afterUid uint64, l *List, readTs uint64) []uint64 {
	...
	l.Iterate(readTs, afterUid, func(p *fb.Posting) error {
...
func getFirst(l *List, readTs uint64) (res *fb.Posting) {
	l.Iterate(readTs, 0, func(p *fb.Posting) error {
		res = p
...
func TestMultiPartListIterAfterUid(t *testing.T) {
	...
	ol.Iterate(uint64(size+1), 50000, func(p *fb.Posting) error {
...
func TestMultiPartListWithPostings(t *testing.T) {
	...
	var labels []string
	err := ol.Iterate(uint64(size)+1, 0, func(p *fb.Posting) error {
		if len(p.Label()) > 0 {
...
func TestMultiPartListDelete(t *testing.T) {
	...
	counter := 0
	ol.Iterate(math.MaxUint64, 0, func(p *fb.Posting) error {
		counter++
		return nil
	})
