...
func parseDirective(it *lex.ItemIterator, schema *pb.SchemaUpdate, t types.TypeID) error {
	...
	case "index":
		...
		schema.VectorSpecs = vectorSpecs
	case "count":
		schema.Count = true
	case "upsert":
		schema.Upsert = true
...
//
// It then defers to parseTokenOrVectorSpec to parse Token.
func parseIndexDirective(it *lex.ItemIterator, predicate string,
	typ types.TypeID) ([]string, []*pb.VectorSpec, error) {
	tokenizers := []string{}
	var vectorSpecs []*pb.VectorSpec
	...
	for {
		tokenText, vectorSpec, sortable, err := parseTokenOrVectorSpec(it, predicate, typ)
		if err != nil {
		...
		if tokenText != "" {
			...
			// parseTokenOrVectorSpec should have returned either
			// non-empty tokenText or non-nil vectorsSpec or an error.
			x.AssertTrue(vectorSpec != nil)
			// At the moment, we cannot accept two VectorSpecs of
...
// parseTokenOrVectorSpec(it, predicate, typ) will parse a "Token" according to the
...
func parseTokenOrVectorSpec(
	it *lex.ItemIterator,
	predicate string,
	typ types.TypeID) (string, *pb.VectorSpec, bool, error) {
	it.Next()
	next := it.Item()
	if next.Typ != itemText {
...
func HasTokenizerOrVectorSpec(update *pb.SchemaUpdate) bool {
	if update == nil {
		return false
	}
	return len(update.Tokenizer) > 0 || len(update.VectorSpecs) > 0
}
