...
func TestSchemaIndexCustom(t *testing.T) {
	...
	require.True(t, State().IsIndexed(x.NamespaceAttr(x.DefaultNamespace, "name")))
	require.False(t, State().IsReversed(x.NamespaceAttr(x.DefaultNamespace, "name")))
	require.Equal(t, "int", State().Tokenizer(x.NamespaceAttr(x.DefaultNamespace, "age"))[0].Name())
