...
func runRestore(t *testing.T, backupLocation, lastDir string, commitTs uint64) map[string]string {
	...
	restored1, err := testutil.GetPredicateValues(
		"./data/restore/p1",
		x.NamespaceAttr(x.DefaultNamespace, "name1"),
		commitTs)
	require.NoError(t, err)

	restored2, err := testutil.GetPredicateValues(
		"./data/restore/p2",
		x.NamespaceAttr(x.DefaultNamespace, "name2"),
		commitTs)
	require.NoError(t, err)

	restored3, err := testutil.GetPredicateValues(
		"./data/restore/p3",
		x.NamespaceAttr(x.DefaultNamespace, "name3"),
		commitTs)
	require.NoError(t, err)
