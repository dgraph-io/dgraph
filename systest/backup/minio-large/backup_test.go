...
func setupTablets(t *testing.T, dg *dgo.Dgraph) {
	...
	for retry := 5; retry > 0; retry-- {
		...
		require.NoError(t, err)
		_, ok1 := state.Groups["1"].Tablets[x.NamespaceAttr(x.GalaxyNamespace, "name1")]
		_, ok2 := state.Groups["2"].Tablets[x.NamespaceAttr(x.GalaxyNamespace, "name2")]
		_, ok3 := state.Groups["3"].Tablets[x.NamespaceAttr(x.GalaxyNamespace, "name3")]
		if ok1 && ok2 && ok3 {
			moveOk = true
			break
		}
