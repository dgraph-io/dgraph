...
func runBackupInternal(t *testing.T, forceFull bool, numExpectedFiles,
	...
	//Flushing the bucketList array, coz we don't want to pass currept data to next imidiate test
	fileList = []string{}
	directoryList = []string{}
	bucketList = []string{}
	S3Config := getEnvs()
	...
	params := testutil.GraphQLParams{
		...
		Variables: map[string]interface{}{
			"dst": backupDst,
			"ak":  S3Config[s3_ACCESS_KEY],
			"sk":  S3Config[s3_SECRET_KEY],
			"ff":  forceFull,
		},
	}
	b, err := json.Marshal(params)
	...
	if err != nil {
		fmt.Println("------------------------------>failed to create a new aws session: ", s3Session)
	}
	s3client := s3.New(s3Session)
	// List all the folders in the bucket.
	s3resp, s3err := s3client.ListObjectsV2(&s3.ListObjectsV2Input{Bucket: aws.String(S3Config[s3_BUCKET_NAME]), Prefix: aws.String(folderNameForCurrentTest + "/")})
	if s3err != nil {
	...
	for _, item := range s3resp.Contents {
		if *item.Key != "manifest.json" {
			...
			fileList = append(fileList, "s3://"+S3Config[s3_REGION]+".amazonaws.com/"+S3Config[s3_BUCKET_NAME]+"/"+*item.Key)

			directoryList = append(directoryList, "s3://"+S3Config[s3_REGION]+".amazonaws.com/"+S3Config[s3_BUCKET_NAME]+"/"+strings.Split(*item.Key, "/")[1])
		}
	}
...
func RemoveDuplicateDirectories(directoryList []string) []string {
	directoryMap := make(map[string]bool)
	for _, item := range directoryList {
		if _, ok := directoryMap[item]; ok {
		} else {
			directoryMap[item] = true
		}
	}
	var result []string
	for item, _ := range directoryMap {
...
func dirCleanup(t *testing.T) {
	...
	resp, err := s3Client.ListObjects(&s3.ListObjectsInput{
		Bucket: aws.String(S3Config[s3_BUCKET_NAME]),
		Prefix: aws.String(folderNameForCurrentTest + "/"),
	})
	for _, key := range resp.Contents {
		_, err = s3Client.DeleteObject(&s3.DeleteObjectInput{Bucket: aws.String(S3Config[s3_BUCKET_NAME]), Key: aws.String(*key.Key)})
		if err != nil {
			exitErrorf("Unable to delete object %q from bucket %q, %v", folderNameForCurrentTest, S3Config[s3_BUCKET_NAME], err)
		}
	}
...
func getEnvs() map[string]string {
	...
	configMap[s3_ACCESS_KEY] = os.Getenv(s3_ACCESS_KEY)
	configMap[s3_SECRET_KEY] = os.Getenv(s3_SECRET_KEY)
	configMap[s3_BUCKET_NAME] = os.Getenv(s3_BUCKET_NAME)
	configMap[s3_REGION] = os.Getenv(s3_REGION)
	if configMap[s3_ACCESS_KEY] == "" || configMap[s3_SECRET_KEY] == "" || configMap[s3_BUCKET_NAME] == "" || configMap[s3_REGION] == "" {
...
func createS3BucketIfNotExists() {
	S3Config := getEnvs()
	if !bucketExists(S3Config) {
		fmt.Println(":( Bucket Dosen't exist, creating the bucket now")
		createDesiredBucket(S3Config)
	}
}

func bucketExists(S3Config map[string]string) bool {
	s3Session, _ := session.NewSession(&aws.Config{
		Region:                        aws.String(S3Config[s3_REGION]),
		CredentialsChainVerboseErrors: aws.Bool(true)},
	)
	...
	bucketNames := strings.Join(bucketList, "")
	return strings.Contains(bucketNames, S3Config[s3_BUCKET_NAME])
}

func createDesiredBucket(S3Config map[string]string) {
	s3Session, _ := session.NewSession(&aws.Config{
		Region:                        aws.String(S3Config[s3_REGION]),
		CredentialsChainVerboseErrors: aws.Bool(true)},
	)
	s3Client := s3.New(s3Session)
	_, bucketCreationError := s3Client.CreateBucket(&s3.CreateBucketInput{
		Bucket: aws.String(S3Config[s3_BUCKET_NAME]),
	})
	if bucketCreationError != nil {
		exitErrorf("Unable to create bucket %q, %v", S3Config[s3_BUCKET_NAME], bucketCreationError)
	}
	// Wait until bucket is created
	fmt.Printf("Waiting for bucket %q to be created...\n", S3Config[s3_BUCKET_NAME])
	bucketCreationWaitError := s3Client.WaitUntilBucketExists(&s3.HeadBucketInput{
		Bucket: aws.String(S3Config[s3_BUCKET_NAME]),
	})
	if bucketCreationWaitError != nil {
		exitErrorf("Error occurred while waiting for bucket to be created, %v", S3Config[s3_BUCKET_NAME])
	}
