...
func sendRestoreRequest(t *testing.T, location, backupId string) int {
	if location == "" {
		location = "/data/backup"
	}
	params := testutil.GraphQLParams{
		Query: `mutation restore($location: String!, $backupId: String) {
			restore(input: {location: $location, backupId: $backupId,
...
func backupRestoreAndVerify(t *testing.T, dg *dgo.Dgraph, backupDir, queryToVerify,
	...
	backup(t, backupDir)
	waitForRestore(t, sendRestoreRequest(t, backupDir, ""), dg)
