...
func AppendAuthInfo(schema []byte, algo, publicKeyFile string) ([]byte, error) {
	...
	authInfo := `# Dgraph.Authorization {"PublicKey":"` + string(keyData) + `","Header":"X-Test-Auth","Namespace":"https://xyz.io/jwt/claims","Algo":"RS256","Audience":["aud1","63do0q16n6ebjgkumu05kkeian","aud5"]}`
	return append(schema, []byte(authInfo)...), nil
}
