...
func AppendJWKAndVerificationKey(schema []byte) ([]byte, error) {
	authInfo := `#Dgraph.Authorization{"VerificationKey":"some-key","Header":"X-Test-Auth",` +
		`"jwkurl":"some-url", "Namespace":"https://xyz.io/jwt/claims","Algo":"algo","Audience":["fir-project1-259e7"]}`
	return append(schema, []byte(authInfo)...), nil
}
