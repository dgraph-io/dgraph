...
func detectRuntime() string {
	...
	if _, err := exec.LookPath("docker"); err == nil {
		out, verr := exec.Command("docker", "--version").Output()
		if verr == nil && strings.Contains(strings.ToLower(string(out)), "podman") {
			return "podman"
		}
		if verr == nil {
			return "docker"
		}
		// The version probe failed (e.g. a podman-docker shim without a
		// working user session); fall through to the podman check below.
	}
