...
func setupTablets(t *testing.T, dg *dgo.Dgraph) {
	require.NoError(t, dg.Alter(context.Background(), &api.Operation{
		Schema: `name1: string .
				 name2: string .
				 name3: string .`}))
	_, err := getHttpClient(t).Get("https://" + testutil.SockAddrZeroHttp + "/moveTablet?tablet=name1&group=1")
	require.NoError(t, err)
	time.Sleep(time.Second)
	_, err = getHttpClient(t).Get("https://" + testutil.SockAddrZeroHttp + "/moveTablet?tablet=name2&group=2")
	require.NoError(t, err)
	time.Sleep(time.Second)
	_, err = getHttpClient(t).Get("https://" + testutil.SockAddrZeroHttp + "/moveTablet?tablet=name3&group=3")
	require.NoError(t, err)
