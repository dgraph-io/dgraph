...
const dateFormatSY = "06"  // time.stdYear
...
func ParseTime(val string) (time.Time, error) {
	if len(val) <= len(dateFormatY) && // Either time.longYear or time.stdYear + month
		(len(val) <= len(dateFormatSY) || val[len(dateFormatSY)] != '-') { // stdYear and no month
