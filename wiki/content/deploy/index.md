...
This guide describes the best practices for setting up a production-ready Dgraph database cluster.
...
1. Dgraph Zero
* port 5080 internal gRPC
* port 6080 external gRPC (admin-only)
2. Dgraph Alpha
* port 7080 internal gRPC
* port 8080 external HTTP
* port 9080 external gRPC
...
Dgraph Zero can live in a private network where communication is only with Dgraph Alphas, database administrators, internal services (such as Jaeger), and possibly developers (see note below). Dgraph Zero's 6080 external port is only necessary for database administrators. For example, it can be used to inspect the database metadata (/state), allocate UIDs or set txn timestamps (/assign), move data shards (/moveTablet), or remove cluster nodes (/removeNode). The full docs about Zero's administrative tasks are in More About Dgraph Zero.
...
Dgraph can use a large number of open file descriptors. Most operating systems set a default lower than what is required.
...
* 2-node cluster with 1 Dgraph Zero, 1 Dgraph Alpha, and replication setting of 1. This means 1 Dgraph Zero, 1 Dgraph Alpha, one replica per Dgraph Alpha group.
* 6-node cluster with  3 Dgraph Zeros, 3 Dgraph Alphas, and replication setting of 3. The Dgraph Alphas are assigned to Alpha Group 1.
* 6-node cluster with 3 Dgraph Zeros, 3 Dgraph Alphas, and replication setting of 1. There's one Dgraph Alpha assigned to Alpha Group 1, one assigned to Alpha Group 2, and one assigned to Alpha Group 3.

Notice that a "6-node cluster" can mean completely different things depending on the number of Dgraph Zeros, number of Dgraph Alphas, and the replication setting.

In the following examples we outline the two most common cluster configurations: a 2-node cluster and a 6-node cluster with 3 Dgraph Zeros, 3 Dgraph Alphas, and a replication setting of 3.
...
Configuration can be set either as command-line flags, environment variables, or in a config file ([Dgraph config docs](https://docs.dgraph.io/deploy/#config)).
...
We provide sample configs for both [Docker Compose](https://github.com/dgraph-io/dgraph/blob/master/contrib/config/docker/docker-compose-ha.yml) and Kubernetes for a 6-node cluster with 3 Alpha replicas per group. You can also run Dgraph directly on your host machines.

A Dgraph cluster can be configured in a high-availability setup with Dgraph Zero and Dgraph Dgraph Alpha each set up with peers. These peers are part of Raft consensus groups, which elect a single leader amongst themselves. The non-leader peers are called followers. In the event that the peers cannot communicate with the leader (e.g., network partition, machine shuts down), the group automatically elects a new leader to continue.

Configuration can be set either as command-line flags, environment variables, or in a config file (Dgraph config docs).
...
The Dgraph Zero group is special in that you must do the following during setup:
* Set unique Raft IDs (`--idx`) per Dgraph Zero. Dgraph will not auto-assign Raft IDs to Dgraph Zero instances.

The first Dgraph Zero that starts will initiate the database cluster. Any following Dgraph Zero instances must connect to the cluster via the `--peer` flag to join. If the `--peer` flag is omitted from the peers, then the Dgraph Zero will attempt to create its own independent Dgraph cluster.
...
Example: `dgraph zero --replicas=3 --idx=1 --my=zero1:5080`
...
Example: `dgraph zero --replicas=3 --idx=2 --my=zero2:5080 --peer=zero1:5080`
...
Example: `dgraph zero --replicas=3 --idx=3 --my=zero3:5080 --peer=zero1:5080`
...
Example: `dgraph alpha --my=alpha1:7080 --zero=zero1:7080`

Second Alpha:

Example: `dgraph alpha --my=alpha2:7080 --zero=zero1:7080`

First Alpha:

Example: `dgraph alpha --my=alpha3:7080 --zero=zero1:7080`
...
These services are not required for a Dgraph cluster to function but are recommended for better insight for operating a Dgraph cluster.
### Metrics and Monitoring
See documentation at https://docs.dgraph.io/deploy/#monitoring.
### Distributed Tracing
See documentation at https://docs.dgraph.io/deploy/#tracing.
