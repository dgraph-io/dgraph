...
{{% notice "note" %}}These instructions will install the latest release version.  To instead install our nightly build see [these instructions]({{< relref "deploy/deploy.md#nightly" >}}).{{% /notice %}}
...
## Step 2: Run Dgraph
{{% notice "note" %}} This is a set up involving just one machine. For multi-server setup, go to [Deploy]({{< relref "deploy/deploy.md" >}}). {{% /notice %}}
...
- Go to [Clients]({{< relref "clients/clients.md" >}}) to see how to communicate
with Dgraph from your application.
- Take the [Tour](https://tour.dgraph.io) for a guided tour of how to write queries in Dgraph.
- A wider range of queries can also be found in the [Query Language]({{< relref "query-language/query-language.md" >}}) reference.
- See [Deploy]({{< relref "deploy/deploy.md" >}}) if you wish to run Dgraph
  in a cluster.
