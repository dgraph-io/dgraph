...
Sentry reporting is on by default. However, starting from v20.03.1 and v20.07.0, there is a configuration flag “enable-sentry” which can be used to completely turn off Sentry events reporting. 
...
2. **Data in Transit:** Events sent from the SDK to the Sentry server is encrypted on the wire with industry-standard TLS protocol with 256 bit AES Cipher.
...
5. **Data Scrubbing**: The Data Scrcubber option (default: on) in Sentry’s settings ensures PII doesn’t get sent to or stored on Sentry’s servers, automatically removing any values that look like they contain sensitive information for values that contain various strings.
