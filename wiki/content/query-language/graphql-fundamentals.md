...
Dgraph Query Language, _DQL_, ([previously named GraphQL+-](https://dgraph.io/blog/post/graphql+-to-dql/)) is based on [GraphQL](http://spec.graphql.org/).  GraphQL wasn't developed for Graph databases, but its graph-like query syntax, schema validation and subgraph shaped response make it a great language choice.  We've modified the language to better support graph operations, adding and removing features to get the best fit for graph databases.
