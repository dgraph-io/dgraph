...
type schedulerCtx struct {
	...
	// stores the taskid of the task which has first mutation for a predicate
	// We don't clear it, number of predicates shouldn't be very high
	predicates map[string]uint32
}

func (s *schedulerCtx) init() {
	s.pending = make(map[string][]uint32)
	s.childTasks = make(map[uint32][]*task)
	s.predicates = make(map[string]uint32)
}
...
func (n *node) submit(t *task) {
	...
	for _, edge := range t.proposal.Mutations.Edges {
		...
		// Check whether this is first mutation, access to predicatesmap is serialized
		// so no lock required
		if _, err := schema.State().TypeOf(edge.Attr); err != nil {
			if tid, ok := n.sctx.predicates[edge.Attr]; !ok {
				// First mutation for given attribute
				n.sctx.predicates[edge.Attr] = t.id
			} else {
				// This is dependent on first mutation of given attribute
				parentsMap[tid] = struct{}{}
			}
		}
		...
		// e) Depends on first mutation if schema is not present
