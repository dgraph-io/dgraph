...
func (n *node) startTask(id op) (*y.Closer, error) {
	...
	case opSnapshot, opIndexing:
		for otherId, otherCloser := range n.ops {
			if otherId == opRollup {
				// Calling delete here so that stopAllTasks doesn't call SignalAndWait
				// again. See GitHub issue #5034. Rollups won't be started for at least
				// another 10 sec, hence, stopTask for this rollup will be a noop and
				// won't delete another entry from n.ops map.
