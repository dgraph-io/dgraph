...
func checkSchema(s *pb.SchemaUpdate) error {
	...
	if typ == types.UidID && s.Directive == pb.SchemaUpdate_INDEX {
		...
		return errors.Errorf("Cannot reverse for non-uid type on predicate %s", x.ParseAttr(s.Predicate))
	}
...
func ValidateAndConvert(edge *pb.DirectedEdge, su *pb.SchemaUpdate) error {
	...
	_, attr, _ := x.ParseNamespaceAttr(edge.GetAttr())
	if x.WorkerConfig.AclEnabled && attr == "dgraph.rule.permission" {
