...
func TestCheckSchema(t *testing.T) {
	...
	// non uid to uid
	s1 := &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "name"), ValueType: pb.Posting_UID}
	require.NoError(t, checkSchema(s1))

	// uid to non uid
	err := schema.ParseBytes([]byte("name:uid ."), 1)
	require.NoError(t, err)
	s1 = &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "name"), ValueType: pb.Posting_STRING}
	require.NoError(t, checkSchema(s1))

	// string to password
	err = schema.ParseBytes([]byte("name:string ."), 1)
	require.NoError(t, err)
	s1 = &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "name"), ValueType: pb.Posting_PASSWORD}
	...
	err = schema.ParseBytes([]byte("name:string ."), 1)
	require.NoError(t, err)
	s1 = &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "name"), ValueType: pb.Posting_FLOAT}
	require.NoError(t, checkSchema(s1))

	// index on uid type
	s1 = &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "name"), ValueType: pb.Posting_UID, Directive: pb.SchemaUpdate_INDEX}
	require.Error(t, checkSchema(s1))

	// reverse on non-uid type
	s1 = &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "name"), ValueType: pb.Posting_STRING, Directive: pb.SchemaUpdate_REVERSE}
	require.Error(t, checkSchema(s1))

	s1 = &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "name"), ValueType: pb.Posting_FLOAT, Directive: pb.SchemaUpdate_INDEX, Tokenizer: []string{"term"}}
	require.NoError(t, checkSchema(s1))

	s1 = &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "friend"), ValueType: pb.Posting_UID, Directive: pb.SchemaUpdate_REVERSE}
	require.NoError(t, checkSchema(s1))

	// Schema with internal predicate.
	s1 = &pb.SchemaUpdate{Predicate: x.NamespaceAttr(x.DefaultNamespace, "uid"), ValueType: pb.Posting_STRING}
	require.Error(t, checkSchema(s1))
