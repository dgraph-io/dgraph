...
func sortWithoutIndex(ctx context.Context, ts *pb.SortMessage) *sortresult {
	...
	for i := 0; i < n; i++ {
		...
		default:
			...
			tempList := codec.ToSortedUids(ts.UidMatrix[i])
			var vals []types.Val
...
BUCKETS:
	...
	for i, ul := range ts.UidMatrix {
		...
		for _, uid := range out[i].ulist.SortedUids {
			present[uid] = true
		}
		for _, uid := range out[i].skippedUids.SortedUids {
			present[uid] = true
		}
		...
		remainingCount := int(ts.Count) - len(r.UidMatrix[i].SortedUids)
