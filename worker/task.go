...
// mergeResults combines results from multiple tablet queries.
// Each tablet returns results only for UIDs it has postings for.
func mergeResults(results []*pb.Result) *pb.Result {
	...
	// Merge by appending UIDs from each tablet's response.
...
func ProcessTaskOverNetwork(ctx context.Context, q *pb.Query) (*pb.Result, error) {
	...
	if len(tablets) > 1 {
		// Fan-out path: send query to all tablet groups in parallel.
		return processTaskFanOut(ctx, q, tablets)
	}
...
// processTaskFanOut sends the query to all tablet groups in parallel
// and merges the results.
func processTaskFanOut(ctx context.Context, q *pb.Query, tablets []*pb.Tablet) (*pb.Result, error) {
	span := trace.SpanFromContext(ctx)
	span.AddEvent("ProcessTaskFanOut", trace.WithAttributes(
		attribute.String("attr", q.Attr),
		attribute.Int("tablets", len(tablets)),
		attribute.String("readTs", fmt.Sprintf("%d", q.ReadTs))))
	...
	for range tablets {
		r := <-ch
		if r.err != nil {
			glog.Warningf("processTaskFanOut(%q): tablet returned error: %v", q.Attr, r.err)
			return nil, r.err
		}
